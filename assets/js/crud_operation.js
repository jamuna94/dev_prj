$(document).ready(function(){
				...
				for(i=0; i<data.length; i++){
					html += '<tr id="'+data[i].id+'">'+
							...
							'<td style="text-align:right;">'+
								'<a href="javascript:void(0);" class="btn btn-info btn-sm editRecord" data-id="'+data[i].id+'" data-name="'+data[i].name+'" data-age="'+data[i].age+'" data-skills="'+data[i].skills+'" data-email="'+data[i].email+'" data-address="'+data[i].address+'">Edit</a>'+' '+
	...
	$('#saveEmpForm').submit('click',function(){
		...
		$.ajax({
			...
			url  : "emp/save",
			dataType : "JSON",
			data : {name:empName, age:empAge, email:empemail, skills:empSkills, address:empAddress},
			success: function(data){
				$('#name').val("");
				$('#skills').val("");
				$('#address').val("");
	...
	// save edit record
	 $('#editEmpForm').on('submit',function(){
		...
		$.ajax({
			...
			url  : "emp/update",
			dataType : "JSON",
			data : {id:empId, name:empName, age:empAge, email:empemail, skills:empSkills, address:empAddress},
			success: function(data){
				$("#empId").val("");
				$("#empName").val("");
				$('#empAge').val("");
